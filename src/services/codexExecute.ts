...
const buildExecutePrompt = (input: ExecuteScenariosViaCodexInput): string => {
  ...
  return [
    ...
    "- For executionMode=full, include fixAttempt details for failures and return pull-request proposal metadata for failed scenarios.",
    "- Include a pull request URL only when you actually created one; otherwise return manual-handoff details with empty url and explicit risk notes. Do not fabricate placeholder URLs.",
    "",
    "Output contract:",
